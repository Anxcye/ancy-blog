...
func NewHTTPServer(cfg *config.Config, logger *slog.Logger) (*HTTPServer, error) {
	...
	api := engine.Group("/api/v1")
	{
		...
		pub := api.Group("/public")
		{
			...
			pub.POST("/analytics/events", container.PublicHandler.AnalyticsEvents)
			pub.GET("/moments", container.PublicHandler.Moments)
			pub.GET("/moments/:id", container.PublicHandler.MomentByID)
			pub.GET("/comments/article/:articleId", container.PublicHandler.CommentByArticle)
			pub.GET("/comments/content/:contentType/:contentId", container.PublicHandler.CommentByContent)
			...
			pub.GET("/comments/content/:contentType/:contentId/total", container.PublicHandler.CommentContentTotal)
		...
		admin.Use(middleware.AuthRequired(container.AuthService))
		{
			...
			admin.GET("/moments/:id", container.AdminHandler.MomentDetail)
			...
			admin.POST("/comments/:id/replies", container.AdminHandler.CommentReply)
			admin.GET("/analytics/overview", container.AdminHandler.AnalyticsOverview)
			admin.GET("/analytics/pages", container.AdminHandler.AnalyticsPages)
			admin.GET("/analytics/visits", container.AdminHandler.AnalyticsVisits)
